...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_channel=23, out_channel=10, zero_init_residual=False):
    ...
    def forward(self, x):
        x = self.conv1(x)       # (b,64,113)
        x = self.bn1(x)         # (b,64,113)
        x = self.relu(x)
        x = self.maxpool(x)     # (b,64,57)

        x = self.layer1(x)
        x = self.layer2(x)      # (b,128,29)
        x = self.layer3(x)      # (b,256,15)
        x = self.layer4(x)      # (b,512,8)

        x = self.avgpool(x)         # (b,512,29)
