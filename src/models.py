...
class visualTransformer(nn.Module):
	def __init__(self,in_channel = 3, kernel_size= 16, encoder_layers = 1):
		super(visualTransformer, self).__init__()
		self.n_window = 5
		self.pacth_embedding = nn.Conv2d(in_channel=in_channel,kernel_size=kernel_size)
		self.pos_encoder = PositionalEncoding(2 , 0.1, self.n_window)
		self.transformer_encoder = TransformerEncoder(encoder_layers, 1)
		self.fc_1 = nn.Sequential(nn.Linear(128, 20), nn.LogSoftmax())
		self.fc_2 = nn.Sequential(nn.Linear(128, 3), nn.LogSoftmax())

	def forward(self, x):
		x = self.pacth_embedding(x)
		x = self.pos_encoder(x)
		x = self.transformer_encoder(x)
		where_x = self.fc_1(x)
		what_x = self.fc_2(x)
		return where_x, what_x

# criterion = nn.CrossEntropyLoss()
# optimizer = optim.Adam(model.parameters(), lr=learning_rate)
#
# where_x, what_x = visualTransformer(input)
#
# loss = criterion(where_x, where_x_target)
# loss = criterion(what_x, what_x_target)