...
def CMMD(source, target, s_label, t_label, kernel_mul=2.0, kernel_num=5, fix_sigma=None):
    s_label = s_label.cpu()         # torch.Size([b])
    b = s_label.shape[0]
    s_label = s_label.view(b, 1)    # torch.Size([b, 1])
    s_label = torch.zeros(b, 31).scatter_(1, s_label.data, 1)      # one-hot编码
    s_label = Variable(s_label).cuda()

    t_label = t_label.cpu()
    t_label = t_label.view(b, 1)
    t_label = torch.zeros(b, 31).scatter_(1, t_label.data, 1)
